...
import type { DashboardStats, AnalyticsChartData, DeviceChartData } from "@/lib/types"
...
export default async function DashboardPage() {
  ...
  try {
    ...
    console.log("All comments result:", {
      ...
      pending: allCommentsData?.filter(c => c.status === "pending").length || 0,
      approved: allCommentsData?.filter(c => c.status === "approved").length || 0,
    ...
    const pendingCommentsCount = commentsCount || commentsData?.length || allCommentsData?.filter(c => c.status === "pending").length || 0
    ...
    const stats: DashboardStats = {
      ...
      activeUsers: profilesData?.filter((u) => u.status === "active").length || 0,
    ...
    return (
      <div className="space-y-4 md:space-y-6">
        {/* Debug panel - törölheted később */}
        <div className="p-3 md:p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <h3 className="font-semibold text-blue-800 text-sm md:text-base">Debug Info</h3>
          <div className="text-xs md:text-sm text-blue-700 grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-4 mt-2">
            <div className="space-y-1">
              <p>Posts: {stats.totalPosts}</p>
              <p>Published: {stats.publishedPosts}</p>
              <p>Draft: {stats.draftPosts}</p>
            </div>
            <div className="space-y-1">
              <p>Users: {stats.totalUsers}</p>
              <p>Active: {stats.activeUsers}</p>
              <p>Pending Comments: {stats.pendingComments}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-6">
          <Card className="w-full">
            <CardHeader className="pb-3 md:pb-6">
              <CardTitle className="text-lg md:text-xl">Látogatói statisztikák</CardTitle>
            </CardHeader>
          ...
          <Card className="w-full">
            <CardHeader className="pb-3 md:pb-6">
              <CardTitle className="text-lg md:text-xl">Eszköz megoszlás</CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
              {/* Chart wrapper with proper mobile dimensions */}
              <div className="w-full h-64 md:h-80">
    ...
    return (
      <div className="space-y-4">
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 md:p-4">
          <h3 className="font-semibold text-red-800 text-sm md:text-base">Dashboard Error</h3>
